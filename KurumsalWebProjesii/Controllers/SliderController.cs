...
namespace KurumsalWebProjesii.Controllers
{
    public class SliderController : Controller
    {
        ...
        [HttpPost] //http post olan actionlistimize -> resim yükleme özelliği kazandırdık!
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "SliderId,Baslık,Aciklama,ResimURL")] Slider slider,HttpPostedFileBase ResimURL)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "SliderId,Baslık,Aciklama,ResimURL")] Slider slider, HttpPostedFileBase ResimURL, int id)
        {
            if (ModelState.IsValid)
            {
                ...
                db.Entry(s).State = EntityState.Modified; // slider yerine s kullanılmalı
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(slider);
